...
| 🧾 증거 | 케이스: 누구의 무엇을 얼마에서 얼마로 바꿨나. 시작 값과 공개 허락이 없으면 경고가 뜬다 |
| ⚡ 블루록 | 단계 · **메타비전** · 능력치 육각형 · 무기 · 주단위 랭크 · **신호** · 무대 테스트 · 흡수 |

블루록에는 층이 세 개다.

| 층 | 재는 것 | 왜 하나/여럿인가 |
|---|---|---|
| **순위** F–S | 주간 산출 개수 **하나** | 여러 개를 섞으면 등급이 등급이 아니게 된다 |
| **능력치** | 몸 · 두뇌 · 산출 · 지속 · 자각 · 흡수 **여섯** | 하나면 어디가 막혔는지 안 보인다 |
| **메타비전** | 예측 도달 vs 실제 도달의 적중률 | 실적은 운으로 나오지만 적중은 운으로 안 나온다 |

**단계는 순서를 강제한다.** 이사기도 여섯 개를 먼저 세우고,
판이 보이기 시작한 뒤에 다이렉트 슛을 뽑았다.

| 단계 | 조건 | 할 일 |
|---|---|---|
| 🧱 균형기 | 최저 축 < 40 | 제일 낮은 축부터 올린다. 무기는 아직 가설 |
| 👁️ 관측기 | 축은 섰고, 예측·실제 짝 < 3 | 올리기 전에 결과를 먼저 적는다 |
| ⚔️ 특화기 | 둘 다 통과 | 하나에 몰아넣는다 |

### 신호는 순서가 있다

| 순위 | 신호 | 무슨 뜻인가 |
|---:|---|---|
| 1 | 📤 공유 | 남이 자기 이름을 걸고 옮겼다 |
| 2 | 🔖 저장 | 다시 꺼내겠다고 했다 |
| 3 | 👀 도달 | 스쳐 갔다 |

도달은 제일 약한 신호인데 제일 눈에 잘 띈다. 그래서 **형식 비교도 무대 테스트도
공유 기준으로 매긴다.** 공유가 아직 없으면 저장으로, 그것도 없으면 도달까지
내려온다 — 없는 신호로는 비교를 못 하니까.

산출물에는 두 가지가 더 붙는다.

- **형식** (목록형 · 비교형 · 역설형 · 혼합형) — 무엇을 말했나가 아니라 어떤 틀에
  담았나. 형식당 3개가 쌓이면 어떤 틀이 퍼지는지 비교가 뜬다.
- **올리기 전 검사 5개** — 통과한 것만 누른다. 4개 이상 통과시킨 게시물과 그 미만이
  실제로 얼마나 갈렸는지를 화면이 되돌려준다. 검사지도 검사받는다.

모든 항목은 목록에서 바로 수정할 수 있다 (아래에서 올라오는 시트).
산출물은 **결과** 버튼으로 공유·저장·도달만 따로 채울 수 있다.
